...
| [5] | ngrok | Yes (2hr session) | Yes | [Download + authtoken](https://ngrok.com/download) | Developer workflows |
| [6] | Local only | Yes | No | None | Same WiFi network only |
...
```cmd
anywhereinput --host 127.0.0.1 --port 8008 --fps 10 --quality 60 --scale 0.5 --tunnel cloudflare
```
...
| `--port` | `8008` | HTTP port |
| `--fps` | `10` | Screen capture FPS (1-30) |
| `--quality` | `60` | JPEG quality 1-95 |
| `--scale` | `0.5` | Stream scale factor (0.1=small, 1.0=full) |
...
```cmd
anywhereinput --host 0.0.0.0 --port 8008 --fps 10 --quality 60 --scale 0.5 --tunnel cloudflare
```
...
| GET | `/api/monitors` | Monitor info → `{"monitors": [...], "current": 0, "auto_track": true}` |
| POST | `/api/monitor/{index}` | Switch capture monitor |
| GET | `/api/token` | Current token → `{"token": "..."}` |
