"""macOS screen capture backend using mss (CGDisplayStream via mss)."""
...
class QuartzScreenCapture(ScreenCaptureBackend):
    """macOS screen capture using mss (which uses CGDisplayStream)."""
    ...
    def _build_engine(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"macOS ScreenCapture rebuilt OK: {self.monitor_count} monitor(s)"
            )
            return True
    ...
    def _get_active_monitor(self) -> MonitorInfo:
        ...
        try:
            ...
            logger.debug("pyautogui position failed, using fallback monitor: %s", e)
