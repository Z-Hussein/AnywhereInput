...
class ScreenCapture:
    ...
    def _draw_cursor(self, draw: ImageDraw.Draw, x: int, y: int) -> None:
        ...
        size = 25  # Increased from 15 for better visibility
        # White outline cross
        draw.line([(x - size, y), (x + size, y)], fill="white", width=4)
        draw.line([(x, y - size), (x, y + size)], fill="white", width=4)
        # Red inner cross
...
class ScreenCapture:
    ...
    def _draw_cursor(self, draw: ImageDraw.Draw, x: int, y: int) -> None:
        ...
        size = 15
        # White outline cross
        draw.line([(x - size, y), (x + size, y)], fill="white", width=3)
        draw.line([(x, y - size), (x, y + size)], fill="white", width=3)
        # Red inner cross
